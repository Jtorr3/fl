...
impl Plugin for Ouroboros {
            ...
            move |egui_ctx, setter, _state| {
                ...
                suite_core::ui::ScaledWindow::new("qeynos-ouroboros-window", Vec2::new(600.0, 680.0))
                    ...
                    .show(egui_ctx, egui_state.as_ref(), |ui| {
                        ...
                        suite_core::ui::PresetBar::new("ouroboros", presets.as_slice()).show(
                            ui,
                            &*params,
                            setter,
    ...
    fn initialize(
        ...
        self.spectrum.init(buffer_config.sample_rate, PluginKind::Generic, "OUROBOROS");
        true
    }
...
mod render_tests {
    ...
    /// formant vocal and a minor-triad pad (2.5 s + 4 s silence tail) as STEREO WAVs →
    /// renders/_audition/OUROBOROS/<QVS_AUDITION_DIR|before>/ — hunt musical vs screeching feedback.
    #[test]
    #[ignore]
    fn audition_render_musical_sources() {
        ...
        let mut vocal = testsig::synth_vocal(220.0, (sr * 2.5) as usize, sr);
        vocal.extend(std::iter::repeat(0.0).take(tail));
        let mut pad = testsig::synth_pad(110.0, 2.5, sr);
        ...
        for (fname, s) in &jobs {
            for (tag, src) in [("vocal", &vocal), ("pad", &pad)] {
                let mut core = OuroCore::new(sr);
                let (l, r) = core.process_stereo(src, s);
                let path = render_path("_audition/OUROBOROS", &format!("{subdir}/{fname}__{tag}"));
